...
public class PageCurlView extends View {
	...
	private ArrayList<Bitmap> mPages = new ArrayList<Bitmap>();
	...
	// reimplement this
	private StandaloneExample standaloneExample;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (!bBlockTouchInput) {
			...
			int width = getWidth();
			tempX = event.getRawX() - oldX;
			tempY = event.getRawY() - oldY;
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (movePoint == 1) {
					if (Math.abs(distanceY) > 10) {
						...
						// check left right to capture image
						getBitmap();
						//
						doFlipo();
					}
	...
	public void setMainView(StandaloneExample example) {
		this.standaloneExample = example;
	}
	...
	private void getBitmap() {
		if (gotBitmap == 0) {
			...
			int w = wm.getDefaultDisplay().getWidth() - 12;
			int h = wm.getDefaultDisplay().getHeight() - 60;
			Log.i("width, height", w + " " + h);
			// get bitmap of current fore view
			View v = standaloneExample.findViewById(R.id.llp);
			b = Bitmap.createBitmap(w, h, Bitmap.Config.ARGB_8888);
			Log.i("bitmap w h", b.getWidth() + " " + b.getHeight());
			Canvas c = new Canvas(b);
			v.layout(0, 0, w, h);
			v.draw(c);
			mBackground = Bitmap.createBitmap(b);
		}

	}

	private void _scrollBy(float y) {
		scrollView.scrollBy(0, ((int) -y));
		isScroll = true;
	}

	private void doFlipo() {
		bUserMoves = true;
	...
	public void FlipAnimationStep() {
		...
		if (mA.x < 1 || mA.x > width - 1) {
			...
			standaloneExample.changeContent();
		} else {
	...
	private void doDynamicCurl() {
		...
		float BH = (float) (FloatMath.sqrt(deltaX * deltaX + deltaY * deltaY) / 2);
	...
	protected void onDraw(Canvas canvas) {
		...
		rect.right = getWidth();
		Log.i("rect", rect.left + " " + rect.bottom + " " + rect.right);
		// First Page render
